...
def get_last_total() -> int:
    ...
    if records:
        ...
        if previous_month_records:
            return previous_month_records[-1]["total"]
...
def get_expenses_stats(monthly_sheet_name: str) -> str:
    ...
    try:
        ...
        return "\n".join(
            f'💸 Расход: {abs(row["amount"]):<5} AZN | 💰 Остаток: {row["total"]:<5} AZN | 📌 {row["description"] or "-"}'
            for row in sheet.get_all_records()
            if TransactionType(row["type"]) == TransactionType.EXPENSE
        )
    except gspread.exceptions.WorksheetNotFound:
        return None
